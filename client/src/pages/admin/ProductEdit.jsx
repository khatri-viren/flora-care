...
const ProductEdit = () => {
  ...
  const handleUpdate = async (productId) => {
    try {
      const response = await fetch(`http://localhost:4000/admin/productedit/${productId}`, {
        method: "PUT",
        headers: {
    ...
    <div className="mx-5 pt-8 lg:mx-20 my-12 text-udark">
      ...
      <div className="imageContainer grid grid-cols-2 gap-10 lg:flex justify-evenly my-5">
        {product.images.map((image, index) => (
          <img key={index} src={`http://localhost:4000/${image}`} alt={`Product ${index + 1}`} className="mx-auto" />
        ))}
      </div>
      <div className="grid lg:grid-cols-2 lg:gap-16">
        ...
        <div className="rightSide">
          ...
          <div className="flex flex-col my-2">
            <button className="py-2 px-4 bg-ubg border border-solid border-udark font-semibold mb-5" onClick={() => handleDelete(id)}>
                Delete Product
            </button>
            <button type="button" className="py-2 px-4 bg-ubg border border-solid border-udark font-semibold" onClick={() => handleUpdate(id)}>
              Update
            </button>
          </div>
        </div>
